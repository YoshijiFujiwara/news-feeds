...
export const getUserFromCookie = (request) => {
  ...
  const expiresInCookie = request.headers.cookie.split(';').find(c => c.trim().startsWith('expiresIn='))
  const userCookie = request.headers.cookie.split(';').find(c => c.trim().startsWith('user='))
  const avatarCookie = request.headers.cookie.split(';').find(c => c.trim().startsWith('avatar='))
